...
\begin{proof}
  ...
  \[ x = -a_{n-1} - a_{n-2} \frac{1}{x}-\ldots - a_0\frac{1}{x^{n-1}}\]

  If $|x|>1$, we have $\abs{-a_{n-1}-\ldots-a_0\frac{1}{x^{n-1}}} \leq 1 \contr$.
...
\begin{proof}[Proof of Theorem 6.1.]
  ...
  \begin{enumerate}
    ...
    For all such $\sigma:L \to \bar{K}$, $\sigam(y)$ satisfies the same monic polynomials as $y$, so is also integral over $\O_K$. Thus $N_{L/K}(y) \in K$ is integral over $\O_K$, and hence $N_{L/K}(y) \in \O_K$.
